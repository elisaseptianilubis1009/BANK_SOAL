...
<body>
                                    ...
                                    $sqll = mysqli_query($conection,"SELECT * FROM tb_essay WHERE id_essay NOT IN(SELECT id_essay FROM tb_pengambilan_essay WHERE id_pengguna='$idP') ORDER BY RAND() LIMIT ".$jumlah."");
                                

                                    //$id = [];
                                    $dapat = mysqli_num_rows($sqll);
                                    if ($dapat  - $jumlah) {
                                        mysqli_query($conection,"DELETE FROM tb_pengambilan_essay WHERE id_pengguna='$idP'");
                            		...
                            		while ($data1 =mysqli_fetch_array($sqll)) {
                            			?>
                            						...
                            						<?php echo $no++?><?php echo ". ".$data1['essay'] ?>
                            						<br>	
                            						         
                                                    <?php 
                                                    $idSoal = $data1['id_essay'];
                                                    mysqli_query($conection, "INSERT INTO tb_pengambilan_essay VALUES('$idP','$idSoal')");
                                                     ?>
